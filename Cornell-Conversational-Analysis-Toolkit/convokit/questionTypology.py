...
class QuestionTypology:
    ...
    def __init__(self, corpus, data_dir, motifs_dir=None,
        ...
        random_seed=0, is_question=None, skip_clustering=False):
    ...
    def get_all_answer_fragments(self, number_of_clusters=8):
        '''
        Added by Alex Crowley for testing
        '''
        all_answer_fragments = []
        for i in range(number_of_clusters):
            target = self.types_to_data[i]
            answer_fragments = target["fragments"]
            fragment_dists = target["fragment_dists"]
            indices_to_return = np.argsort(fragment_dists)
            for j in indices_to_return:
                all_answer_fragments.append(answer_fragments[j])
        return all_answer_fragments

    def get_all_question_motifs(self, number_of_clusters=8):
        '''
        Added by Alex Crowley for testing
        '''
        all_question_motifs = []
        for i in range(number_of_clusters):
            ...
            motifs = target["motifs"]
            motif_dists = target["motif_dists"]
            indices_to_return = np.argsort(motif_dists)
            for j in indices_to_return:
                all_question_motifs.append(motifs[j])
        return all_question_motifs
...
class QuestionClusterer:
    ...
    def extract_clusters(matrix_dir,km_file,k, d, snip, verbose, norm, idf, leaves_only,
        ...
        print(len(mtx_obj['q_terms']))
        print(len(mtx_obj['a_terms']))
